...
>
> **代码风格约定**：`templates/` 下的组件统一用 **ES2015+**（const/let、模板字面量、箭头函数）。两个例外是刻意为之的轻量降级：`404.html` 与 `bgm.html` 用 ES5（`var`/`function`）以保持「最小自包含、无引擎依赖」——这几乎是裸拷即用的单块，不引共享引擎。`examples/minimal.html` 作为「双击即跑」的演示页也用 ES5。复制到生产项目时，除非注明理由，否则跟模板本体的 ES2015+ 保持一致。
