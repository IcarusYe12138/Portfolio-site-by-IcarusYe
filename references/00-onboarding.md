# 00 · 首次接洽：问卷、素材与工具链

> **本 skill 被首次调用时，必须先走本流程**——先问清楚，再动手。宁可多问一轮，不要凭空假设用户的审美和内容。
> 黄金规则：**信息缺失时反问用户，不擅自代答。** 每个未回答的问题都可能变成后期的大返工。
> **分批读取**：本文件也按「一问一答」推进——先用户的**场景**（从零 / 改已有站 / 加语言 / 修可达性）定位到下面对应的接洽分支，只问该分支需要的问题，**不要把全部问卷一次性倒给用户**；素材与工具链的完整清单供你按需取用，不必整页背诵给用户。

## 一、风格接洽问卷（必须逐项问）

用户说「帮我做个作品集网站」时，第一轮对话只问风格与参考：

1. **你有没有代码参考？**（现成网站仓库、开源模板、以前的站点代码）
2. **你有没有网站截图？**（喜欢的网站、海报、品牌页的截图）
3. **你有没有 HTML 文件？**（旧站导出、原型页、半成品）
4. **你有没有图片或 Figma 之类的设计稿？**（情绪板、设计系统、Figma/Penpot 链接）

拿到任何一种 → 按 `01-design-and-style.md` 的「找风格三步法」走：参考素材 + 用户专业方向 → 风格 Demo → 书面设计规范。
什么都没有 → 引导用户口头描述 + 反面清单（「不要什么」往往比「要什么」更有用），先出一版风格 Demo 再对齐。

## 二、内容接洽问卷（第二轮，风格定了就问内容）

1. **简历是什么样的？** 教育背景、工作/实习经历、技能栈、语言——有没有现成 CV 文件？
2. **作品集是什么样的？** 有哪些作品、什么类型（影像/音频/网页/方案/设计）、每件作品有没有介绍文案？
3. **有没有包含所需要的链接？** 每件作品的外链（原始发布页、云盘直链、在线文档）是否齐全？**海外链接与内地链接是否都齐？**（双链路要求见 `03-media-compat.md`）

### 素材不全时的标准话术

如果作品链接/文件还没备齐：

> 链接和文件不齐没关系。我们可以**先做偏向设计风格的 Demo**（用占位内容把版式、配色、组件定下来），风格锁定后你再慢慢补传文件和链接，内容层最后填。

——顺序永远是：**风格 Demo 先行，内容后补**。不要等素材齐了才动工，也不要拿真内容去试错版式。

### 内容组织的推荐载体（引导用户三选一）

作品清单别散落在聊天记录里，引导用户集中记录在：

| 载体 | 适合 | 优点 |
|---|---|---|
| **[飞书](https://www.feishu.cn/)文档**（多维表格更佳） | 团队协作 / 手机随时改 | 结构化字段（作品名/类型/年份/海外链接/内地链接/简介）天然对齐双链路需求 |
| **详细 Markdown 文件** | 技术型用户 / 仓库党 | 可进 git、可 diff、可生成 |
| **Excel 表格** | 数据量大 / 表格控 | 列即字段，一行一作品 |

核心要求：**每件作品一行，链接列齐**（海外一列、内地一列），后续增删有据可查（配合 `10-content-ops.md`）。

## 三、工具链推荐（告知用户，不强制）

### 构建工具：优先推荐 TRAE（TraeCN）※ 作者验证过（截至 2026-08）

官网：https://www.trae.cn/

推荐理由：
- **本地实时多设备预览**：手机 / 平板 / 桌面视口即时切换，做多模态适配非常直观——响应式断点（`05-structure-i18n.md` 的 4 断点体系）可以边改边看；
- 主流模型可选，代码质量与迭代效率有保障；
- 当然不限制其他工具——本 skill 的方法论与工具无关，任何 agent/编辑器都适用。

> **同类替代**（具名推荐只是「作者验证过的优先项」，不是唯一解）：构建类只要是支持本地实时预览的 agent 编码环境都行（Cursor / VS Code + Copilot / Claude Code / 其他 TRAE 变体）；下面的设计原型同理。**工具是会过期的时点选择**——若推荐的官网链接失效或产品下线，按「同类能力」而非「品牌」去替换即可。

### 设计原型：Web 视觉 Demo 首选 Kimi K3 ※ 作者验证过（截至 2026-08）

介绍页：https://www.kimi.ai/blog/kimi-k3

推荐理由：审美水平在当前可用于网页设计的模型里足够好，适合承接「找风格三步法」第 2 步——把参考截图 + 用户气质描述喂给它，产出属于用户自己的风格 Demo（配色/字体/母题/版式）。
Demo 定稿后回到 TRAE 用本 skill 的组件规范落地代码。

同类替代（审美强的多模态/生图模型均可）：Stitch（见 `01-design-and-style.md`「设计阶段资源」）、或用任何支持「图片+联网」的 Agent 产 Demo——关键是会生图 + 能复述设计语言，不绑死某一款。

**分工记忆口诀：Kimi 出审美，TRAE 出工程。**

### MCP（可选，agent 提效）

如果用户的构建 agent 支持 MCP，按需推荐接入：

- **[github-mcp-server](https://github.com/github/github-mcp-server)**（官方）——让 agent 直接操作 GitHub 仓库：建站代码托管、PR / release 管理、迭代文档进库都能在对话里完成；
- **[mcp-server-cloudflare](https://github.com/cloudflare/mcp-server-cloudflare)**（官方）——让 agent 直接查配额、管理 Workers / R2 等资源，配合 CF Pages 部署链路排障顺手；
- **[AnySearch](https://www.anysearch.com/home)**——通用搜索增强：查托管平台配额变动、字体许可、域名价格对比等「建站过程中的情报问题」比内置搜索更稳，agent 不用切浏览器就能核实事实。

不装也不影响本 skill 的任何流程——它们只是把「切到浏览器手动操作」这步省掉。

### 多模态理解（可选，视频/音频/图片检索）

构建 agent 若**不具备视觉能力**（不能直接看图/听音），接 **[Qwen-MM-Plugins](https://github.com/QwenLM/Qwen-MM-Plugins)** 可以让它理解视频、音频、图片——作品集场景里对齐「这张海报长什么样」「这段视频的节奏」时很有用。需要用户自己去申请并接入千问（Qwen）API key。属于锦上添花，不接也不影响流程。

### Logo 快速生成（网页工具，风格探索期用）

没有参考素材、又想快速试 logo 方向时，可用网页版生成器探索（**仅作分享，不做保证**——产出需自行确认版权与商用许可）：

- **[明日方舟：终末地风格 Logo 生成器](https://ark.ncreeper.top/)**——工业感/游戏 UI 向字标，适合硬朗风格；
- **[图叙（TuxuAI）logo 生成](https://www.tuxuai.com/share/inspiration?shareId=880)** 及其[灵感页](https://www.tuxuai.com/inspiration?categoryId=31_14&imageId=7216)——通用向，风格谱系更宽。

生成结果只作**风格探索的输入**（配色/字形/母题的参照），最终站点视觉仍走「找风格三步法」（`01-design-and-style.md`）收敛成书面 Style Spec。

**安装纪律（MCP 与 skill 通用，硬规则）**：
- **绝不擅自安装**——任何 MCP / skill 的安装动作，必须先征得用户明确许可；agent 只给命令、理由与影响，由用户自己执行，或用户明确授权后才可代为执行；
- 推荐时说明「不装不影响主流程」，把选择权完全留给用户。

### 伴侣 skill 推荐（接洽时必问）

本 skill 管**作品集的垂直生命周期**（结构、媒体、三语、运维、部署），但「编码时的审美判断」「无障碍合规审计」是通用设计能力的活——由伴侣 skill 补位。接洽时按下面的分层表**询问用户要不要安装**；标 ★ 的强烈建议装（本 skill 的工作流默认它们兜底品味与合规）：

装机口碑列只给「高 / 中 / 低」分级（**不写具体星数**——数字随时间波动且难跨平台对齐，分级足以判断装不装）：

| 层 | Skill | 装机口碑 | 补什么位 | 安装 |
|---|---|---|---|---|
| ★ 必装 | **frontend-design**（Anthropic 官方） | 高 | 写码前锁定美学方向、反「Inter + 紫渐变」AI slop——正好接住本 skill「风格 Demo → 组件落地」的落地段 | `claude plugin add anthropic/frontend-design`；TRAE 用户在插件市场装 Frontend Design 插件 |
| ★ 必装 | **web-design-guidelines**（Vercel） | 高 | 100+ 条 WCAG 2.2 / UX 规则自动审计——比本 skill 11 号总检单的人工 a11y 段更硬 | `npx skills add https://github.com/vercel-labs/agent-skills --skill web-design-guidelines` |
| 推荐 | **ui-ux-pro-max** | 高 | 240+ 风格库 / 127 字体配对 / 99 UX 准则——用户没有参考素材、纯靠描述定风格时的发散器 | `npx skills add https://github.com/nextlevelbuilder/ui-ux-pro-max-skill` |
| 推荐 | **impeccable** | — | brand/product 双模式 + 23 个精修命令（typeset/colorize/bolder/quieter）——作品集属 brand 模式 | https://impeccable.style/ |
| 可选 | **high-end-visual-design** | 中 | 视觉精修层，Tailwind 栈增益最大 | 各 skill 市场搜索 |
| 可选 | **webapp-testing**（Playwright 类） | — | 让 agent 在真浏览器里自测响应式——可替代一部分「多设备人工过一遍」 | ComposioHQ/awesome-codex-skills |

**推荐话术**（接洽第 0 步问完工具链后）：

> 「为了让代码层的设计品味和无障碍有兜底，建议装两个伴侣 skill：Anthropic 官方的 frontend-design（管审美方向）和 Vercel 的 web-design-guidelines（管 WCAG 审计）。要我现在把安装命令给你，还是你已有同类能力？」

**纪律（重要）**：
- **先问再推，绝不擅自安装**——用户可能已有同类能力（TRAE 自带设计插件、或已装 ui-ux-pro-max），重复安装无益；且任何安装动作须用户明确许可（见上文「安装纪律」）；
- **skill 预算 ≤ 20–30 个**（Anthropic 官方建议）：装太多触发判定互相打架、准确率反而暴跌——只推表里的，不鼓励囤积；
- 伴侣 skill 是「品味与合规」的横向能力，**不替代本 skill 的垂直流程**——冲突时（如伴侣 skill 想上 React 组件库），以本 skill 的「零框架静态优先」原则为准。

### 按需主动推荐（需求触发式，建站全程生效）

接洽期问一次「要不要装伴侣 skill」还不够——**用户在实操中冒出的具体需求，才是推荐的最好时机**。全程留意下表的触发信号，命中就**主动开口介绍 + 问要不要用/装**，经许可才行动；用户不感兴趣就不再提第二次。

| 用户需求信号 | 主动推荐 | 详情出处 |
|---|---|---|
| 「要放社交媒体 / 品牌图标」 | simple-icons（★ 首选）等图标库 | `01` §五 |
| 没参考素材 / 想快速出多屏原型 | Google Stitch、Design Skills Hub | `01` §五 |
| 「想放数据 / 图表」 | lieflat-charts | `01` §五 |
| 「要不要装 XX 设计 skill」 | Design Tokens（★）/ identity-skill / Motion 评估结论 | `01` §五 |
| 做电子名片 / 嵌入名片 iframe | Popl（海外）/ muse link（内地）双源 | `02-components.md` 嵌入名片 |
| 大文件放不下（PDF/视频/音频） | 对象存储（腾讯 COS / 阿里 OSS / Cloudflare R2） | `03-media-compat.md` |
| 想快速试 logo 方向 | 网页版 logo 生成器 | 本文件 §三 |
| 要让 agent 直接操作 GitHub / Cloudflare | github-mcp-server / mcp-server-cloudflare | 本文件 §三 |
| 要查托管配额 / 字体许可 / 域名价格 | AnySearch | 本文件 §三 |
| agent 看不了用户的图 / 视频 / 音频 | Qwen-MM-Plugins | 本文件 §三 |

> 三条硬规则：① **主动介绍 ≠ 擅自安装**——推荐时给出处、理由与「不装不影响主流程」，安装必须用户点头；② 一个需求只推表内最相关的 1–2 项，不整表倾倒；③ 触发信号没出现就不主动推，避免打扰。


## 四、隐私红线（每次接洽都要自检）

- 只问**建站必需**的信息；联系方式、真实姓名、证件号等敏感信息不主动索要，仅在用户自愿提供用于页面内容时接收；
- 用户提供的个人信息**只进站点项目**（本地/私有仓库），**绝不进本 skill 仓库**、不进文档示例、不进提交信息；
- skill 文档中的链接一律用 `overseas.example` / `mainland.example` 占位；
- 示例文案不使用可识别到具体个人的内容。

## 五、接洽完成判据（开工前 checklist）

- [ ] 风格输入确认（参考素材 ≥1 种，或明确「无参考、靠 Demo 对齐」）
- [ ] 语言需求确认（单语 / 双语 / 三语？——决定是否启用 `templates/trilingual.html`）
- [ ] 地域需求确认（仅海外 / 海外+内地双链路？——决定是否走 `03-media-compat.md`）
- [ ] 作品清单载体确认（飞书 / Markdown / Excel）
- [ ] 简历素材到位或已约定「后补」
- [ ] 工具链告知完毕（TRAE 构建 + Kimi K3 设计原型；可选 MCP / 多模态 / logo 生成器按需提及，**未擅自安装任何 MCP 或 skill**）
- [ ] 伴侣 skill 已询问（★ 两个必装项：装了 / 已有同类 / 用户明确拒绝，三态记录）
- [ ] 隐私红线自查通过

全部打勾 → 进入 `SKILL.md` 的「推荐工作流」第 1 步。
