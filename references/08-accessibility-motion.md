...
## 一、基线：WCAG 2.2 一致性级别与测试工具

### 一致性级别（本 skill 的基线 = AA）

WCAG 2.2（2023-10 成为 W3C 正式推荐标准）共 86 条成功标准，分三级：

| 级别 | 条数 | 定位 |
|---|---|---|
| **A** | 31 | 最低线——缺失即不可用 |
| **AA** | 24 | **本 skill 基线**（含全部 A）——正文对比度 4.5:1、焦点可见、键盘可达都出自这级；多数法规（ADA、欧盟 EAA）以此为合规底线 |
| **AAA** | 31 | 增强标准，**不作为通用要求**（W3C 明示部分内容无法满足），只按需取用（如大字号文本对比度 7:1） |

注：WCAG 2.2 已移除 4.1.1 Parsing——现代 HTML5 语义化写法自动满足，无需专门检查。

### 测试工具栈

| 工具 | 平台 | 成本 | 用途 |
|---|---|---|---|
| **NVDA** | Windows | 免费 | 屏幕阅读器主力测试（市场份额最大） |
| **VoiceOver** | macOS / iOS | 系统内置 | 苹果端辅助验证（`Cmd+F5` 开关） |
| JAWS | Windows | 商业 | 企业场景补充，个人站可跳过 |
| **拔掉鼠标** | 全平台 | — | 纯键盘 Tab / Enter / Esc 走全站——最低成本、最高收益的一步 |
| **200% 缩放** | 全平台 | — | 浏览器缩放到 200%，版式不得散架或丢内容 |

上线前至少跑：键盘全程 + NVDA 或 VoiceOver 任一 + 200% 缩放。**自动化工具（Lighthouse / axe）只能覆盖约三成问题**，屏幕阅读器实测不可省。

### Landmarks 与语义结构

每页显式使用语义标签，屏幕阅读器可按区跳转：`<header>`（页头）/ `<nav>`（导航）/ `<main>`（主内容，每页唯一）/ `<footer>`（页脚）。skip 链接的 `href` 应指向 `<main>` 内的锚点（见下节）。

### 富媒体无障碍（作品集站特有）

| 媒体 | 要求 |
|---|---|
| 视频作品 | 配字幕（captions，AA 硬要求）；含关键视觉信息时补音频描述或文字说明 |
| 音频作品 | **配文字稿（transcript）**——页面或外链均可，但必须可达；纯音频卡片至少给出内容摘要 |
| iframe 嵌入 | `title` 必填（下文 ARIA 表） |
| 图片作品 | `alt` 描述内容；纯装饰 `alt=""` + `aria-hidden` |

作品集大量依赖音视频——**transcript / 字幕缺失是无障碍审计里最常见的硬伤**，接媒体时（`03-media-compat.md`）就要一并准备，不要留到上线前。
...
- [ ] landmarks 齐备：header / nav / main（唯一）/ footer；skip 链接指向 main
- [ ] 视频有字幕；音频作品有 transcript 或内容摘要
- [ ] 屏幕阅读器抽测一页（NVDA 或 VoiceOver）+ 200% 缩放不散架
