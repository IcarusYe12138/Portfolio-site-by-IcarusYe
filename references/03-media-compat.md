...
简中版与海外版的差异**不只是「链接不同」**，完整设计要覆盖四层，逐层对齐：

| 层 | 内容 | 处理机制 |
|---|---|---|
| ① 链接地址 | CV、网盘文件、文章原站、公司官网 | 字典 href 键（`cvHref` 等）/ 索引页卡片用 `data-href-en`/`data-href-zh` 属性对 |
| ② iframe 源 | 电子名片、视频嵌入、在线 deck | `data-langs` 整块切换（连同容器换媒体源，绝不只换文字）；iframe 自身 src 经字典键在 setLang 时同步 |
| ③ 渠道提示文案 | 「在 Dropbox 听」vs「在微云听」、「本地存档 — 全文」vs「原站 — 全文」 | 字典普通键，一句话的事；诚实告知渠道差异，不要伪装成同一来源 |
| ④ 嵌入策略 | 海外 `<video>` 直链（`raw=1` + poster）vs 内地平台 iframe（`?embed`） | 同一作品两套呈现，`data-langs` 版块各挂一套；懒加载参数各配各的 |

**审计要点**：做完区域适配后按层逐项过——最容易漏的是**第 ③ 层**：链接换了、iframe 换了，但文案还写着「在 Dropbox 听」。四层全部收敛在「字典 + data 属性」体系里，DOM 结构跨语言同构，检查时可以按 `data-` 属性名 grep 全量核对。
