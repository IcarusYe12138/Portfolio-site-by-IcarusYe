...
> ⚡ **现在就做（快速上手，3 步）**——正在第一次用本 skill 的人，从这三步开始，不用看任何前置文档：
> 1. **确认场景**：从零建站 / 改已有站 / 加语言 / 解决「内地打不开、媒体播不了」——告诉我哪一种；
> 2. **要三样东西**：① 风格参考（1–3 张截图，或一句话形容；没有就先跳过）② 作品清单（标题+类型+年份+链接，一行一件）③ 一个目标；
> 3. **按需开文档**：读过下文「接入与分批读取」，只开当前任务需要的参考文献，**不整包读**——省 token、少噪音。
>
> 信息缺失就反问，绝不擅代答；绝不擅自安装任何 MCP / skill（见 `00-onboarding.md`）。

本 skill 提炼自构建 [icarusye.site](https://icarusye.site/) 的实战经验——一个已上线的三语（EN / 简中 / 繁中）个人作品集网站：纯手写 HTML/CSS/JS + Cloudflare Pages，零框架零构建，含大量音视频与图文作品，同时保障国际与中国内地双平台可达。该站点既是这套方法论的活示例，也可作为阅读本文档时的参照。（**不必参考它**也能启动——它只是一份来源，不是门槛。）

**它不是一套固定模板，而是一套方法论**：设计规范、组件规范、跨地域媒体策略、部署策略、迭代节奏与避坑清单。把「具体实现」抽象成「可迁移的原则 + 可复用的代码片段」。

---

## 接入与分批读取（怎么用本 skill，最省 token）

**铁律：按需分批、逐层推进，绝不一次性读光站点的全部文件，也绝不一次打开全部 references。** 每个环节用上一步的结论判断「要不要继续」，需要才读下一步。

分两类接入：

### A. 新站从 0 到 1（推荐顺序）
每步完成后再进下一步，各自只开对应参考文献：
1. **接洽**（→ `00-onboarding.md`）：问风格 + 内容 + 载体 + 工具链，信息缺失就问；
2. **定风格**（→ `01`）：出风格 Demo → 锁 Style Spec；
3. **定结构**（→ `05`）：页面清单 + 语言机制；
4. **铺组件**（→ `02` + `templates/`）：复制骨架套 token；
5. **接媒体**（→ `03`）：双链路配对；
6. **部署**（→ `04`）：托管 + 域名 + 缓存；
7. **迭代/内容/上线**（→ `06/07/10/11` 按需）。

### B. 接已有网站（改版 / 加语言 / 修可达性）
按「分层最小样本」读**用户自己站点**，不做全量扫描：
1. **入口层**：主页 / 目录 / `_headers` / `robots.txt` / `sitemap.xml` → 判断「什么站、什么语言、大致结构」；
2. **样式层**：CSS 变量 / 设计 token → 判断风格是否成体系；
3. **脚本层**：语言切换 / 筛选 / 播放器 → 判断三语机制与组件复用；
4. **媒体层**：遇到视频/音频/外链才查对应资源可达性；
5. 只深读当前任务对应的 references，不碰无关章节。

> 省 token 三原则：① 一次只推进一个环节；② 一文只开对应 references；③ 读用户站点只用「分层最小样本」，不整包读。这也让不懂技术的人更好懂——你不用理解全部，只要回答我该环节的问题。
...
> 完整分批顺序见上文「接入与分批读取 · A」。下面是每步的展开要点；**一次只推进一步，只开对应 references**。
...
2. **定结构**（→ `05`）：页面清单 / 首页架构 / assets 拆分 / 语言机制先定型——后期改结构成本最高。
3. **铺组件**（→ `02` + `templates/`）：复制骨架套 Style Spec 的 token；`examples/minimal.html` 最小组装参考。
4. **接媒体**（→ `03`）：视频双链路、音频自托管、外链文章本地存档，一次配对配全。
5. **部署上线**（→ `04`）：直传托管、`_headers`、robots/sitemap/og:image、JSON-LD、域名、（可选）统计；部署后 ITDOG 抽测两地可达性，再真机过一遍。
6. **迭代打磨**（→ `06` + `07`）：三层文档 + 书面迭代一轮一轮来，改后过坑清单。
7. **内容增删**（→ `10`）：照同步清单执行 + `tools/audit.sh` 计数审计。
8. **上线收尾**（→ `11`）：跑 audit.sh + 过一页总检单，全部打勾再发布。
