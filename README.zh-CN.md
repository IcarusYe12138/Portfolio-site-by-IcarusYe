...
提炼自一个已上线的作品集：纯手写 HTML/CSS/JS + Cloudflare Pages，零框架、零构建，大量使用音频、视频、在线嵌入文档与长图文，全部内容在防火墙两侧均可正常访问。

**这不是一套拿来即抄的模板，而是一套方法论**：设计规范、组件规格、双地域媒体策略、部署纪律、迭代节奏，以及一份 23 条真实翻车记录的避坑清单，每条都按「现象 → 根因 → 怎么避免」格式沉淀。
...
| 多语言站慢慢长成几套分叉代码 | 单 DOM 三语引擎：内嵌字典 + `data-i18n` + `?lang=` URL + 浏览器检测 |
| 迭代混乱、改着改着就散了 | 书面迭代轮次：每个决定先锁进决策日志，再动代码，一轮一次执行完 |
...
├── references/
│   ├── 01-design-and-style.md    设计 token + 「找风格」方法论
│   ├── 02-components.md          组件规格：顶栏、卡片、筛选、播放器、磁贴场、滚动条
...
1. **定风格**：收集作者喜欢的参考截图，交给任意支持图片的 agent，产出属于作者自己的风格 Demo（配色、字体、母题、版式）。绝不照抄现有网站。
2. **定结构**：页面清单、assets 布局、语言机制——结构越晚改越贵。
3. **铺组件**：从 `templates/` 复制骨架，套入 Demo 的 token。
4. **接媒体**：视频双链路、音频自托管、文章本地存档，一次配全。
5. **上线**：直传托管、`_headers`、robots、sitemap、og:image。
6. **书面迭代**，每轮结束过一遍避坑清单。
